...
const meta = getBacktestLessonMeta('bt2');

const CONTENT = {
  kh: {
    finishUnlocked: '✓ បញ្ចប់មេរៀន',
    scenarioTag: 'សេណារីយ៉ូ ២ នៃ ៤',
...
export default function Backtest2({ onNavigate, onDone }) {
  const { lang } = useLanguage();
  const t = CONTENT[lang];
  const { videos } = useVideos();
  const src = videos['bt-scenario-2']?.url;
